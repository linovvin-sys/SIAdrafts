(function () {
  ...
  function check() {
    ...
    timer = setTimeout(async () => {
      try {
        const r = await fetch(
          `/SIAdrafts/Backend/api/Enrollment/check_enrollment.php?student_id=${encodeURIComponent(sid)}&school_year=${encodeURIComponent(sy)}&semester=${encodeURIComponent(sem)}`
        );
