...
$stmt = $pdo->prepare(
    "SELECT s.student_id, s.student_name, s.first_name, s.last_name, s.middle_name,
            s.section_id, sec.section_name, st.type_name
     FROM student s
     JOIN section sec     ON s.section_id = sec.section_id
     JOIN student_type st ON s.type_id = st.type_id
     WHERE s.student_id = ?"
);
$stmt->execute([$enroll['student_id']]);
$student = $stmt->fetch();
...
$params = array_merge([$enroll['school_year'], $enroll['semester'], $student['section_id']], $ids);
...
$type_row = $pdo->prepare("SELECT type_name FROM student_type WHERE type_id = ?");
$type_row->execute([$enroll['type_id']]);
$type_name = $type_row->fetchColumn() ?: 'Regular';

// Default due date: 7 days from now
$default_due  = (new DateTime())->modify('+3 days')->format('Y-m-d');
$page_scripts = ['js/enrollment-finalize.js'];
...
function student_fullname(array $s): string {
    $ln = $s['last_name'] ?? ''; $fn = $s['first_name'] ?? ''; $mn = $s['middle_name'] ?? '';
    return ($ln && $fn) ? $ln . ', ' . $fn . ($mn ? ' ' . $mn : '') : ($s['student_name'] ?? '');
}
  ...
  <div class="row justify-content-center">
    <div class="col-12 col-lg-8">
      ...
      <div class="reg-form-wrap">
        ...
        <!-- Payment section -->
        <div class="reg-section no-print">
          <h3 class="reg-section-title">Payment Details</h3>
          <div class="alert-box alert-error mb-3" v-if="error">
            <iconify-icon icon="mdi:alert-circle-outline"></iconify-icon>
            {{ error }}
          </div>
          <div class="pay-grid">
            <div>
              <label class="form-label fw-bold small">Amount Due (PHP)</label>
              <input type="number" v-model="amount_due" class="form-control"
                     placeholder="0.00" min="0" step="0.01">
            </div>
            <div>
              <label class="form-label fw-bold small">Down Payment (PHP)</label>
              <input type="number" v-model="downpayment" class="form-control"
                     placeholder="0.00" min="0" step="0.01">
            </div>
            <div>
              <label class="form-label fw-bold small">Balance</label>
              <input type="text" :value="balance" class="form-control" readonly>
            </div>
            <div>
        ...
        <div class="reg-section print-only">
          ...
          <div class="reg-info-grid">
            <div class="reg-info-row">
              <span class="ri-label">Amount Due</span>
              <span class="ri-value">PHP _______________</span>
            </div>
            <div class="reg-info-row">
              <span class="ri-label">Down Payment</span>
              <span class="ri-value">PHP _______________</span>
            </div>
            <div class="reg-info-row">
              <span class="ri-label">Balance</span>
              <span class="ri-value">PHP _______________</span>
            </div>
            <div class="reg-info-row">
              <span class="ri-label">Due Date</span>
              <span class="ri-value">_______________</span>
            </div>
          </div>
        </div>
