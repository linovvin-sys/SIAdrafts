...
$page_scripts = ['js/dup-enrollment-check.js'];
...
$student_query = "
    SELECT s.student_id, s.student_name, s.first_name, s.last_name, s.middle_name,
           s.section_id, s.type_id AS default_type_id,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare($student_query . " WHERE s.student_id = ?");
...
// Detect year level from section name (e.g. BSIT-2A → 2)
$detected_year = 1;
if ($student) {
    preg_match('/-(\d+)/i', $student['section_name'], $m);
    $detected_year = (int)($m[1] ?? 1);
}
...
$types = $pdo->query("SELECT type_id, type_name FROM student_type ORDER BY type_id")->fetchAll();
