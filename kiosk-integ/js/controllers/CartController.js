...
var lastOrder = reactive({ orderId: '', customerName: '', paymentMethod: '', items: [], subtotal: 0, vat: 0, total: 0 });
...
var CartController = {
  ...
  checkout(cart, customerName, paymentMethod) {
    ...
    lastOrder.items = cart.map(function (item) { return { name: item.name, price: item.price, quantity: item.quantity }; });
