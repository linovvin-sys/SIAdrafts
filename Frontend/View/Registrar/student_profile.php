...
if ($student) {
    ...
    $stmt = $conn->prepare("
        SELECT CONCAT('Payment received — ₱', FORMAT(p.downpayment, 2)) AS label, p.paid_at AS ts, p.balance
        FROM payment p
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        $balance = (float)$row['balance']; // most recent payment row's balance carries forward
    }
    $stmt->close();
